/* ---------------------------------------------------------------
 * Test for FIR kernel with 8-bit real input and output
...
alignas(8) int8_t buffer_aie_in[NUM_INPUT_WORDS + AIE_POINTER_ADJUST];
alignas(8) int8_t buffer_aie_out[NUM_OUTPUT_WORDS];
...
int main()
	...
	for (int rd_iter = 0; rd_iter < (NUM_INPUT_WORDS + AIE_POINTER_ADJUST); rd_iter++)
		{
		fscanf(fp, "%d\n", &file_buffer);
		window_writeincr((output_window_int8 *)&aie_input, file_buffer);
		}

	// Send the window of data to the kernel
	fir_asym_dual8t_8x8real(get_input_window_int8(&aie_input), get_output_window_int8(&aie_output));
